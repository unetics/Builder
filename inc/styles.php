...
function siteorigin_panels_style_dialog_form(){
	...
	if(empty($fields)) {
		"Your theme doesn't provide any visual style fields. ";
		return;
	}
	...
	foreach($fields as $name => $attr) {
		...
		switch($attr['type']) {
			case 'select':
				?>
				<select name="panelsStyle[<?= esc_attr($name) ?>]" data-style-field="<?= esc_attr($name) ?>" data-style-field-type="<?= esc_attr($attr['type']) ?>">
					<?php foreach($attr['options'] as $ov => $on) : ?>
						<option value="<?= esc_attr($ov) ?>"><?= esc_html($on) ?></option>
					<?php endforeach ?>
				</select>
				<?php
				break;
			...
			case 'checkbox' :
				...
				<label class="siteorigin-panels-checkbox-label">
					<input type="checkbox" name="panelsStyle[<?= esc_attr($name) ?>]" data-style-field="<?= esc_attr($name) ?>" data-style-field-type="<?= esc_attr($attr['type']) ?>" />
					Enabled
				</label>
				<?php
				break;
			...
			case 'number' :
				?><input type="number" name="panelsStyle[<?= esc_attr($name) ?>]" data-style-field="<?= esc_attr($name) ?>" data-style-field-type="<?= esc_attr($attr['type']) ?>" /> <?php
				break;

			default :
				?><input type="text" name="panelsStyle[<?= esc_attr($name) ?>]" data-style-field="<?= esc_attr($name) ?>" data-style-field-type="<?= esc_attr($attr['type']) ?>" /> <?php
				break;
		}
		echo '</p>';
	}
